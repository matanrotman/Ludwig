...
- **The ribbon is ALWAYS left-to-right, never mirrored.** This reverses the earlier "mirrors in RTL" decision (line 23) after seeing it built. The user's reasoning: the ribbon, like the sidebar, belongs to the *application frame*, not to the page — even though its buttons act on the current page. Flipping the whole strip when a page's direction changes would move every control under the user's hands. Word behaves the same way. Implemented in `ribbon_menu.dart` as a hardcoded `Directionality(textDirection: ltr)`.
