...
**Last updated:** 2026-07-16 r2 (floating-toolbar pointer-anchor round 2 shipped — fixed Cmd+A never showing, timing, RTL mirroring; required an editor-fork core-file change, user-authorized; awaiting the user's live verdict. Also: Google Drive backup Stages 1–2 built, live-verified, committed in a parallel session — snapshots are landing on the user's real Drive.)
...
## Active feature
**Google Drive backup** (`specs/google-drive-backup.md`) — **in progress, ~3-session build.** Session 1 (Stages 1–2: engine + scheduling) is done, live-verified, and committed. **Session 2 starts at Stage 3 (Backup settings page)** — the spec's "Stage 3 — detailed plan" section has exact file names and core touches, no re-exploration needed. Session 3 = Stage 4 (restore) + Stage 5 (drills).
...
- **Floating selection toolbar — round 2 SHIPPED to the dock app 2026-07-16 (r2), awaiting the user's live verdict.** Round 1 this session (pointer-anchor experiment: mouse pointer when over the visible selection, else a third of the way down the visible selection, else clamp into the viewport) had real bugs the user caught live: Cmd+A never showed the toolbar at all (both with the mouse over the text and over the sidebar), drag/selection felt too fast/flickery, and RTL positioning wasn't direction-aware. Root-caused and fixed all three — **this round required touching the editor fork's core `floating_toolbar.dart`, not just app code** (user explicitly authorized this after Claude Code's own permission system blocked a first silent attempt):
  - **Root cause of "Cmd+A never shows the toolbar":** a genuine pre-existing bug in the fork's own `FloatingToolbar._showToolbar()` — its visibility-suppression check (`top <= floatingToolbarHeight`) was an indirect, buggy proxy that trips unconditionally whenever `_findSuitableRect` returns its `Rect.zero` "nothing visible" sentinel (`Rect.zero`'s top always collapses to exactly `floatingToolbarHeight` downstream). This runs in the OUTER wrapper *before* our app code is ever invoked, so no app-side fix could reach it. Fixed: `_findSuitableRect` now does a real viewport-overlap check (was: excluded any rect starting even fractionally above the viewport, regardless of how much of it was visible below), and the suppression check now tests the `Rect.zero` sentinel directly instead of the buggy proxy. **Honesty note:** extensive attempts to build a widget test that fails pre-fix and passes post-fix did not succeed (consistent with this project's standing lesson that geometry/scroll bugs often don't reproduce in widget tests) — the fix is shipped on code-reasoning confidence, not a proven failing-first test; the existing 4-test suite in the fork's `floating_toolbar_test.dart` still passes clean.
  - **Timing:** selection-driven debounce bumped 200ms → 400ms (both repos' test suites needed matching updates — `pumpAndSettle()` doesn't reliably wait out a plain `Timer` once the widget tree stops scheduling frames on its own, so tests now pump an explicit duration past the debounce instead of relying on incidental settle timing).
  - **RTL/LTR mirroring + sidebar clearance (app-side):** reused the codebase's existing `documentPopoverDirection` convention (same `AppearanceSettingsCubit.layoutDirection` source of truth) — LTR opens toolbar to the right of the cursor, RTL to the left; both directions now keep a 16px margin off the editor's own edge so the toolbar never sits flush against it.
  - Fork: commit `80136613` on `rtl-direction-aware-selection-menu`, pushed to `origin`, pin resynced in the app's `pubspec.yaml`/`pubspec.lock` (no drift). App: `toolbar_pointer_tracker.dart`, `desktop_floating_toolbar.dart`, `editor_page.dart`, both widget test files. 8/8 app-side widget tests pass (mock `AppearanceSettingsCubit` added to both test files for the new RTL read). **Kill-switch `anchorToolbarToPointer = false`** in `toolbar_pointer_tracker.dart` still restores pre-experiment behavior exactly. **PR #1222 still needs NO update** — confirmed again by diff: contains only the debounce/timing fix, zero positioning/gate logic.
...
## Next step
1. **Google Drive backup — build Stage 3 (Backup settings page).** `specs/google-drive-backup.md` → "Stage 3 — detailed plan" has exact new file names, core-touch list, and the reusable seams (FilePickerService, SettingsBody/SettingsCategory pattern, dialogs.dart) already located — a new session can start directly from that section without re-exploring the codebase. Then Stage 4 (guarded restore + RESTORE.md — full state machine already written out in the spec) and Stage 5 (drill checklist, also in the spec). Until Stage 4's restore drill passes, keep treating backups as unverified insurance (see top section).
2. **Floating toolbar pointer-anchor round 2 — user live-test and verdict.** Shipped to the dock app 2026-07-16 r2 (re-open the app). Test in a **disposable scratch page**: forward/backward drag (toolbar near where the mouse finished, opens right of cursor in LTR / left in RTL); Cmd+A on a long page with the mouse over the selected text (toolbar at the mouse) and with the mouse elsewhere (toolbar ~a third down the visible selection, near the top, NOT flush against the sidebar) — this specifically was broken last round, now has a real fork-level fix; Shift+arrow selection with the mouse parked elsewhere; scrolling with the toolbar up; does the ~400ms delay feel right, not flickery but not sluggish. Bad verdict → flip `anchorToolbarToPointer = false` in `toolbar_pointer_tracker.dart` + rebuild (app-side only), or revert both the app commit and the fork commit `80136613` + re-pin (see "Bug status" above for exactly what changed).
