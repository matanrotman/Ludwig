...
- **✅ Upstream toolbar PR SENT — https://github.com/AppFlowy-IO/appflowy-editor/pull/1222** (opened 2026-07-15 r2 at the user's explicit instruction). Branch `fix/floating-toolbar-debounce-race`, commit `7299f9d4`, 1 file, +24/−9. The debounce-key split + post-frame deferral, rebased onto upstream's newer `dispose()`. If accepted, `floating_toolbar.dart` leaves the fork for good — one less file to reconcile at every future merge.
  - **⚠️ BLOCKED ON THE USER: the CLA is unsigned.** CI reports `license/cla — pending: Contributor License Agreement is not signed yet`. It cannot merge until the user signs at https://cla-assistant.io/AppFlowy-IO/appflowy-editor?pullRequest=1222 — a legal agreement, so only they can do it.
...
3. **Toolbar PR — sign the CLA, then watch CI.** [PR #1222](https://github.com/AppFlowy-IO/appflowy-editor/pull/1222) is open but **cannot merge until the user signs the CLA** (https://cla-assistant.io/AppFlowy-IO/appflowy-editor?pullRequest=1222 — only they can). After that, upstream's Flutter 3.38.5 CI is the first real compile this fix has ever had; if it fails, fix on the branch and push. Maintainers may ask for a reproducing test the PR already admits doesn't exist.
