...
**Last updated:** 2026-07-16 (two parallel sessions: Google Drive backup Stages 1–2 built, live-verified, committed — snapshots are landing on the user's real Drive; separately, the floating-toolbar pointer-anchor experiment shipped to the dock app, awaiting the user's live verdict)
...
- **Floating selection toolbar — pointer-anchor experiment SHIPPED to the dock app 2026-07-16, awaiting the user's live verdict.** The selection-end anchor regressed Cmd+A on a long page (toolbar rendered at the document's last block, below the viewport). New behavior (user-designed via interview): anchor at the **mouse pointer when it's over the visible selection**; otherwise about **a third of the way down the selection's visible span** ("near the top, not too top"); safety-net clamp into the viewport if no selection rect is visible. All app-side (`toolbar_pointer_tracker.dart` new, `desktop_floating_toolbar.dart`, `editor_page.dart`); **kill-switch `anchorToolbarToPointer = false`** in the new file restores the old behavior exactly. 8/8 widget tests pass (3 new), incl. one proving the translucent Listener receives raw pointer events during the editor's own drag gesture. **PR #1222 needs NO update** — verified by diffing the branch: it contains only the debounce/timing fix, zero positioning logic; this bug never lived in the PR. Worst case: flip the kill-switch or revert the one commit.
...
2. **Floating toolbar pointer-anchor — user live-test and verdict.** Shipped to the dock app 2026-07-16 (re-open the app). Test in a **disposable scratch page**: forward/backward drag (toolbar near where the mouse finished); Cmd+A on a long page with the mouse over the selected text (toolbar at the mouse) and with the mouse elsewhere (toolbar ~a third down the visible selection, near the top); Shift+arrow selection with the mouse parked elsewhere; scrolling with the toolbar up. Bad verdict → flip `anchorToolbarToPointer = false` in `toolbar_pointer_tracker.dart` + rebuild, or revert the commit.
