...
**Last updated:** 2026-07-16, end of session (floating-toolbar pointer-anchor experiment is DONE — the user confirmed "very happy," 4 rounds of live-tested fixes landed, small unspecified quirks remain, see "Bug status". Google Drive backup Stage 3 was committed by the parallel session (`9abdb7c4b`); that session appears to be mid-Stage-4 as of this writing — see "Where things stand".)
...
- **Decision (user, 2026-07-16): replace the safety net with automatic backups to Google Drive** — `specs/google-drive-backup.md`. **Stages 1–2 are BUILT, live-verified, and committed (`a37665df3`) — the app is right now snapshotting into the user's real Google Drive** (`My Drive/AppFlowy Backups/snapshots/`, every 30 min when changed + on quit + a 60s post-launch catch-up). No Google auth code — the already-running Drive desktop app does the uploading. **Stage 3 (settings page) is BUILT and committed (`9abdb7c4b`).** Stage 4 (restore) appears to be mid-flight in a parallel session as of this writing (`lib/shared/backup/restore_service.dart` + its test exist on disk, uncommitted) — verify that session's own state before assuming it's done. Stage 5 (drills) is next after that.
...
## Active feature
**Google Drive backup** (`specs/google-drive-backup.md`) — **in progress, ~3-session build.** Session 1 (Stages 1–2: engine + scheduling) is done, live-verified, and committed. Session 2 (Stage 3: Backup settings page) is done and committed (`9abdb7c4b`). **A session appears to be mid-Stage-4 (restore) right now** — `restore_service.dart` + its test exist uncommitted on disk; a fresh session should check `git status`/`git log` and that session's own progress before assuming where it left off. The spec's "Stage 4 — detailed plan" has the full restore state machine already written out if it needs picking up from scratch. Then Stage 5 (drills).
...
- **Toolbar work is fully committed** (`e9616c11f`, `8943e5466`, `f30231389` on the app side; `80136613`, `5354a98d` on the fork, pushed). **A parallel session appears to be mid-Stage-4 backup work as of this writing** — `restore_service.dart` + its test exist uncommitted on disk; deliberately left alone this session (neither reviewed nor committed — that's that session's call to make when it wraps up).
...
## Next step
1. **Google Drive backup — check in on Stage 4 (guarded restore + RESTORE.md).** Stage 3 is done and committed. A parallel session left `restore_service.dart` + its test uncommitted on disk — a new session should check whether that session finished and wrapped up, or whether it needs picking up. `specs/google-drive-backup.md` → "Stage 4 — detailed plan" has the full restore state machine already written out (idle → confirmed → preRestoreSnapshot → extracting → validating → swapping → awaitingRelaunch) if it needs starting fresh. Wire the Stage-3 "Restore…" buttons (currently disabled placeholders in `settings_backup_view.dart`) to the new flow. Then Stage 5 (drill checklist). Until Stage 4's restore drill passes, keep treating backups as unverified insurance (see top section).
