...
**Last updated:** 2026-07-16 r4 (floating-toolbar pointer-anchor round 3 shipped — fixed round 2's own bugs: pointer-over-page too narrow, RTL fallback anchor pinned to the far-left wall, delay tuned to 300ms; awaiting the user's live verdict. Google Drive backup Stage 3 — Backup settings page — was built and has since been committed by the parallel session (`9abdb7c4b`); check that session's own status notes for its current state, not this line.)
...
2. **Floating toolbar pointer-anchor round 3 — user live-test and verdict.** Shipped to the dock app 2026-07-16 r4 (re-open the app). Test in a **disposable scratch page**: forward/backward drag (toolbar near where the mouse finished, opens right of cursor in LTR / left in RTL); Cmd+A with the mouse anywhere inside the page — even not over highlighted text — should now anchor at the mouse; Cmd+A with the mouse genuinely outside the editor (e.g. over the sidebar) should fall back to ~a third down the visible selection, near the top, and should NOT be pinned to the far-left edge in RTL pages (this was round 2's specific new bug); Shift+arrow selection with the mouse parked elsewhere; scrolling with the toolbar up; does the 300ms delay feel right. Bad verdict → flip `anchorToolbarToPointer = false` in `toolbar_pointer_tracker.dart` + rebuild (app-side only), or revert the app commit and the fork commits `5354a98d`+`80136613` + re-pin (see "Bug status" above for exactly what changed each round).
