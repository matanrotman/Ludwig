...
**Last updated:** 2026-07-16 (cloud-sync mystery solved — it's dead by design; Google Drive backup spec written, sign-off pending)
...
- **Decision (user, 2026-07-16): replace the safety net with automatic backups to Google Drive** — `specs/google-drive-backup.md`, scoped and interviewed, **sign-off pending, no code yet**. Approach needs NO Google auth code: write snapshot zips into the already-running Google Drive desktop mount (`~/Library/CloudStorage/GoogleDrive-matan.rotman@gmail.com`). Phase 2 (multi-device sync via the same folder) deliberately deferred.
- Until backups exist: **keep `~/Desktop/AppFlowy_data_backup_backup_20260715_0925`** and consider a manual copy of `data_dev_beta.appflowy.cloud` at the end of any heavy writing day.
...
## Active feature
**Next up: Google Drive backup** (`specs/google-drive-backup.md`) — spec written 2026-07-16, awaiting the user's sign-off before code.
RTL/LTR support (`specs/rtl-support.md`) continues in parallel: Phase 1 (sidebar) done and signed off. Phase 2 (document-content direction) in good shape: auto-detect direction, bidi, block-insert menu direction, icon-margin gap, and the **empty-line cursor** are all done. New this session: bidi arrow-key navigation scoped (root cause pinned; visual-movement behavior decided; own-session-sized). Also new: `specs/plugin-system.md` (stub — the user wants to entertain a real plugin system; investigation groundwork recorded, interview not started).
...
## Next step
1. **Google Drive backup — get spec sign-off, then build Phase 1** (`specs/google-drive-backup.md`). This replaces the cloud as the safety net and is now the top priority: until it ships, the user's writing has no automatic second copy.
2. **Shortcuts: user homework + two known bugs.** The user should try Settings → Shortcuts (search/rebind/conflict detection already ship; persisted to `<data>/shortcuts/shortcuts.json`) and report real gaps. Known gaps: no H4–H6 bindings; typed character shortcuts (`# ` → heading) aren't rebindable; persistence stores one binding per key with no platform field. Two real bugs found 2026-07-16, fix when this becomes active: (a) a cleared binding silently returns after restart (`clearCommand()` persists `command: ''` but `updateCommand('')` no-ops on load — editor fork `command_shortcut_event.dart:99`); (b) commands with `getDescription: null` render as blank rows in the settings list. Good news: new shortcuts are app-side only (`editor_plugins/shortcuts/command_shortcuts.dart:19`) and appear in the settings UI automatically — zero fork changes.
3. **Mid-character cursor bug** — capture a real-render screenshot of the caret right after the comma in an embedded date, get the user's judgment on correct placement, fix to match, un-skip the `caret_bidi_test.dart` case.
4. **Toolbar PR #1222 — CLA now SIGNED (2026-07-15), but upstream CI has not run** ([PR](https://github.com/AppFlowy-IO/appflowy-editor/pull/1222) shows only the CLA check; a maintainer likely must approve first-contributor workflows). When CI runs, it's the first real compile this fix has ever had — check it. Maintainers may ask for a reproducing test the PR already admits doesn't exist.
5. **Automate the three footguns** — a single wrapper script for real-target tests: run the test → `flutter build macos --debug` (restore the dock app from the test build) → `defaults delete … path_location` → verify by contents (`grep -c IntegrationTestWidgetsFlutterBinding` = 0). Doing these by hand has failed before.
6. **Bidi arrow-key navigation** (`specs/rtl-support.md`, scoped 2026-07-16) — visual caret movement; own-session-sized; editor-fork change, possibly upstream-worthy.
7. **Editor-fork upstream merge — do NOT schedule.** Deferred indefinitely; see the merge section for the trigger. Listed here only so its absence isn't read as an oversight.
